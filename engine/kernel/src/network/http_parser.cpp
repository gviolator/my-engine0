...
namespace {
constexpr char KeyValue_Separator = ':';
}

//-----------------------------------------------------------------------------
HttpParser::Header::Header() = default;

HttpParser::Header::Header(std::string_view key_in, std::string_view value_in) :
    key(key_in),
    value(value_in)
{
}
...
bool HttpParser::Header::operator==(std::string_view rhs) const
{
    return strings::icaseEqual(this->key, rhs);
}
...
HttpParser::HeaderIterator::HeaderIterator(const HttpParser& parser_, const Header& value_) :
    parser(&parser_),
    value(value_)
{
}
...
{
    MY_DEBUG_ASSERT(value, "Invalid http header iterator operation");
    MY_DEBUG_ASSERT(parser);

    if (value = HttpParser::getNextHeader(parser->m_buffer, value); !value)
    {
        parser = nullptr;
    }
...
HttpParser::Header HttpParser::tryParseHeader(std::string_view line)
{
    const auto index = line.find(KeyValue_Separator);
    if (index == std::string_view::npos)
    {
        return {};
    }

    const std::string_view key = line.substr(0, index);
    const std::string_view value = line.substr(index + 1, line.size() - index - 1);
    return Header{strings::trim(key), strings::trim(value)};
}

Result<HttpParser::Header> HttpParser::parseHeader(std::string_view line)
{
    HttpParser::Header header = tryParseHeader(line);
    if (!header)
    {
        return MakeError("Invalid http header:({})", line);
    }

    return header;
}

std::string_view HttpParser::getNextLine(std::string_view buffer, std::string_view line)
{
    ...
    MY_DEBUG_ASSERT(offset >= 0 && offset < static_cast<ptrdiff_t>(buffer.size()));

    const std::string_view::size_type end1 = buffer.find(EndOfLine, static_cast<std::string_view::size_type>(offset));
    if (end1 == std::string_view::npos)
    {
        return {};
    }

    const std::string_view::size_type begin = end1 + EndOfLine.size();
    const std::string_view::size_type end = buffer.find(EndOfLine, begin);
...
HttpParser::Header HttpParser::getFirstHeader(std::string_view buffer)
{
    if (buffer.empty())
    {
        return {};
    ...
    const std::string_view::size_type end1 = buffer.find(EndOfLine);
    const std::string_view firstLine = (end1 == std::string_view::npos) ? buffer : std::string_view{buffer.data(), end1};

    if (Header firstHeader = tryParseHeader(firstLine))
    {
        return firstHeader;
    }

    const std::string_view nextLine = getNextLine(buffer, firstLine);
    return tryParseHeader(nextLine);
}

HttpParser::Header HttpParser::getNextHeader(std::string_view buffer, const Header& header)
{
    if (!header)
    {
        return {};
    ...
    return tryParseHeader(getNextLine(buffer, header.key));
}

HttpParser::Header HttpParser::findHeader(std::string_view buffer, std::string_view key)
{
    for (Header header = getFirstHeader(buffer); header; header = getNextHeader(buffer, header))
    {
        if (header == key)
        {
            return header;
...
HttpParser::Header HttpParser::headerAt(std::string_view buffer, size_t index)
{
    ...
    MY_DEBUG_FATAL_FAILURE("Invalid header index: ({}), headersCount: ({})", index, counter);
    return {};
}
...
HttpParser::HttpParser(std::string_view buffer) :
    m_buffer(HttpParser::getHeadersBuffer(buffer))
{
    if (m_buffer.empty())
    {
        return;
    ...
    MY_DEBUG_ASSERT(m_buffer.size() <= buffer.size());
...
{
    return {*this, getFirstHeader(m_buffer)};
}
...
bool HttpParser::hasHeader(std::string_view header) const
{
