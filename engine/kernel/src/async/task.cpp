...
struct AwaiterState
{
    ...
    static IAllocator& getAwaiterStateAllocator()
    {
        constexpr Byte StateBlockSize = nearestPowerOf2(sizeof(AwaiterState));

        static auto g_awaiterStateAllocator = createFixedSizeBlockAllocator(createHostVirtualMemory(512_b, true), StateBlockSize, true);
        return *g_awaiterStateAllocator;
    }
    ...
    */
    ...
    void deleteDelayed()
    {
        ...
        {  // This code path should be executed only for tests, when there is no default executor.
            ...
            MY_DEBUG_FATAL(isSafeToDestroyInplace);

            AwaiterState* self = this;
            std::destroy_at(self);
            getAwaiterStateAllocator().free(self);

            return;
        }

        MY_DEBUG_FATAL(executor);
