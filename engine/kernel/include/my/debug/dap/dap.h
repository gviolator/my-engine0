...
/*
    Замечание по поводу использования optional<string> вместо string.
    В принципе с точки зрения логики (сериализации) пустое значение для string эквивалентно optional none,
    но при сериализации пустая строка ВСЕГДА будет сохраняться, тогда как
    optional может быть представлен как null, так и вообще отсутстовать (что предпочтительнее с точки зрения экномии трафика между клиентом и адаптером):
    т.е.
    в случае string = "":
    {
        "memoryReference": ""
    ...
    в случае optional<string> = none (и нужных настроек подсистемы сериализации):
    {
    }
*/
...
inline constexpr const char* const kCommand_Initialize = "initialize";
inline constexpr const char* const kCommand_Attach = "attach";
inline constexpr const char* const kCommand_Launch = "launch";
inline constexpr const char* const kCommand_ConfigurationDone = "configurationDone";
inline constexpr const char* const kCommand_SetBreakpoints = "setBreakpoints";


inline constexpr const char* const kEvent_Initialized = "initialized";
inline constexpr const char* const kEvent_Stopped = "stopped";
...
struct ProtocolMessage
{
    ...
    static constexpr std::string_view kMessageType_Request{"request"};
    static constexpr std::string_view kMessageType_Event{"event"};
    static constexpr std::string_view kMessageType_Response{"response"};

#pragma region Class info

    MY_CLASS_FIELDS(
        CLASS_FIELD(seq, my::serialization::RequiredFieldAttribute{}),
        CLASS_FIELD(type, my::serialization::RequiredFieldAttribute{}))
#pragma endregion
...
{
#pragma region Class info
    MY_CLASS_BASE(ProtocolMessage)

    MY_CLASS_FIELDS(
        CLASS_FIELD(command, my::serialization::RequiredFieldAttribute{}),
...
#pragma region Class info
    ...
    MY_CLASS_FIELDS(
        CLASS_NAMED_FIELD(eventType, "event", my::serialization::RequiredFieldAttribute{})
        // CLASS_FIELD(body)
    )
#pragma endregion
...
template <typename Body = RuntimeValuePtr>
struct GenericEventMessage : EventMessage
{
#pragma region Class info

    MY_CLASS_BASE(EventMessage)
...
struct InitializedEvent : EventMessage
{
    MY_CLASS_BASE(EventMessage)

    InitializedEvent(unsigned messageSeq);
};
...
#pragma region Class info
    ...
    MY_CLASS_FIELDS(
        CLASS_FIELD(request_seq, my::serialization::RequiredFieldAttribute{}),
        CLASS_FIELD(success, my::serialization::RequiredFieldAttribute{}),
        CLASS_FIELD(command),
        CLASS_FIELD(message))
#pragma endregion
    ...
    */
    ...
    ResponseMessage(unsigned messageSeq, RequestMessage&& request);
...
#pragma region Class info
    MY_CLASS_FIELDS(
        CLASS_FIELD(clientID),
        CLASS_FIELD(clientName),
        CLASS_FIELD(adapterID),
        CLASS_FIELD(locale),
        CLASS_FIELD(linesStartAt1),
        CLASS_FIELD(columnsStartAt1),
        CLASS_FIELD(pathFormat),
        CLASS_FIELD(supportsVariableType),
        CLASS_FIELD(supportsVariablePaging),
        CLASS_FIELD(supportsRunInTerminalRequest),
        CLASS_FIELD(supportsMemoryReferences),
        CLASS_FIELD(supportsProgressReporting),
        CLASS_FIELD(supportsInvalidatedEvent),
        CLASS_FIELD(supportsMemoryEvent),
        CLASS_FIELD(supportsArgsCanBeInterpretedByShell),
        CLASS_FIELD(supportsStartDebuggingRequest),
        CLASS_FIELD(supportsANSIStyling))
#pragma endregion
    /**
...
#pragma region Class info
    MY_CLASS_FIELDS(
        CLASS_FIELD(supportsConfigurationDoneRequest),
        CLASS_FIELD(supportsFunctionBreakpoints),
        CLASS_FIELD(supportsConditionalBreakpoints),
        CLASS_FIELD(supportsHitConditionalBreakpoints),
        CLASS_FIELD(supportsEvaluateForHovers),
        CLASS_FIELD(supportsSetVariable),
        CLASS_FIELD(supportsRestartFrame),
        CLASS_FIELD(supportsGotoTargetsRequest),
        CLASS_FIELD(supportsStepBack),
        CLASS_FIELD(supportsRestartRequest),
        CLASS_FIELD(supportsTerminateRequest),
        CLASS_FIELD(supportsTerminateDebuggee),
        CLASS_FIELD(supportsLoadedSourcesRequest),
        CLASS_FIELD(supportsLogPoints),
        CLASS_FIELD(supportsExceptionOptions),
        CLASS_FIELD(supportsValueFormattingOptions),
        CLASS_FIELD(supportsExceptionInfoRequest),
        CLASS_FIELD(supportsCompletionsRequest),
        CLASS_FIELD(supportsModulesRequest),
        CLASS_FIELD(supportsDelayedStackTraceLoading),
        CLASS_FIELD(supportsReadMemoryRequest),
        CLASS_FIELD(supportsWriteMemoryRequest),
        CLASS_FIELD(supportsDataBreakpoints),
        CLASS_FIELD(supportsInstructionBreakpoints),
        CLASS_FIELD(supportsClipboardContext))
#pragma endregion

    /* Client supports the 'configurationDone' request. */
    ...
    /* Adapter supports hit-conditionals on breakpoints. */
    std::optional<bool> supportsHitConditionalBreakpoints;

    /* Adapter supports evaluating expressions in hover. */
    std::optional<bool> supportsEvaluateForHovers;

    /* Adapter supports setting a variable's value. */
    ...
    std::optional<bool> supportsTerminateDebuggee;
    ...
    /* Adapter supports exceptionOptions on exceptions. */
    std::optional<bool> supportsExceptionOptions;

    /* Adapter supports value formatting options (ValueFormat/StackFrameFormat). */
    ...
    /* Adapter supports clipboard context for evaluate requests. */
...
struct DebugConfigurationBase
{
#pragma region Class info
    MY_CLASS_FIELDS(
        CLASS_FIELD(type),
        CLASS_FIELD(name),
        CLASS_FIELD(request, my::serialization::RequiredFieldAttribute{}))
#pragma endregion
    ...
    /* The request type of the debug session. ("attach" or "launch")*/
    std::string request;
};
...
#pragma region Class info
    MY_CLASS_FIELDS(
        CLASS_FIELD(algorithm, my::serialization::RequiredFieldAttribute{}),
        CLASS_FIELD(checksum, my::serialization::RequiredFieldAttribute{}))
#pragma endregion
...
#pragma endregion
    ...
    */
    ...
    static bool EqualFast(const Source& source, std::string_view path);

    static bool EqualFast(const Source& source1, const Source& source2);
...
#pragma endregion
    ...
    */
    ...
    RuntimeValuePtr data;

    static GenericEventMessage<OutputEventBody> MakeEvent(unsigned seq);

    OutputEventBody() = default;

    OutputEventBody(std::string output, std::string category = "console");
};
...
#pragma endregion
    ...
    */
    ...
    static GenericEventMessage<TerminatedEventBody> MakeEvent(unsigned seq);
};
...
#pragma endregion
    ...
    */
    ...
    StoppedEventBody(std::string reason);

    StoppedEventBody(std::string eventReason, std::string eventDescription);

    StoppedEventBody& SetBreakpoint(unsigned id)
    {
        if (!hitBreakpointIds)
        {
            hitBreakpointIds.emplace();
        }
        hitBreakpointIds->push_back(id);
        return *this;
    }
};
...
#pragma endregion
    ...
    */
    ...
    Scope(std::string_view scopeName, unsigned varRef = 0);
};
...
#pragma endregion
    ...
    */
    ...
    Variable(std::string_view variableName, std::string_view variableValue, unsigned refId = 0);

    Variable(std::string_view variableName, unsigned refId);
};
...
#pragma endregion
    ...
    static EvaluateResponseBody FromVariable(dap::Variable);
};
