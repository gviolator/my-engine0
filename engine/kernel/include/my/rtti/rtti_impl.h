...
public:
    ...
    RttiClassSharedState(IAllocator* allocator_, AcquireFunc, DestructorFunc, std::byte* ptr);
...
struct MY_KERNEL_EXPORT RttiClassStorage
{
    ...
    static void* allocateStateAndInstance(void* inplaceMemBlock, IAllocator* allocator, size_t size, size_t alignment, AcquireFunc, DestructorFunc);
    ...
    static T* instanceFactory(void* inplaceMemBlock, IAllocator* allocator, Args&&... args)
    {
    ...
    template <typename T, typename... Args>
    static T* createInstanceWithAllocator(IAllocator* allocator, Args&&... args)
    {
        if (!allocator)
        {
    ...
    {
        return instanceFactory<T>(nullptr, &getDefaultAllocator(), std::forward<Args>(args)...);
    }
};
...
Ptr<Interface_> createInstanceWithAllocator(IAllocator* allocator, Args&&... args)
{
    using namespace ::my::rtti_detail;
...
public:                                                                        \
    ...
    {                                                                          \
        RttiClassStorage::getSharedState(*this).addInstanceRef();              \
                                                                               ...
                                                                               \
    void releaseSelf() noexcept override                                       \
    {                                                                          \
    ...
    {                                                                          \
        return RttiClassStorage::getSharedState(*this).getInstanceRefsCount(); \
                                                                               ...
                                                                               \
    const ::my::IAllocator* getRttiClassInstanceAllocator() const           \
    {                                                                          \
