...
inline void decompose(const Matrix4& mat, Vector3* translationOut, Quat* rotationOut, Vector3* scaleOut)
{
		...
		rowLen = length(row[2]);
		row[2] /= rowLen;
		skew.setY(skew.getY() / length(row[1]));
		skew.setX(skew.getX() / rowLen);
...
inline void decompose(const Matrix4& mat, Vector3& translationOut, Quat& rotationOut, Vector3& scaleOut)
{
    decompose(mat, &translationOut, &rotationOut, &scaleOut);
}
...
//----------------------------------------------------------------------------
inline bool operator == (const Vectormath::Vector2 &u, const Vectormath::Vector2 &v) { return (u.getX() == v.getX() && u.getY() == v.getY()); }
inline bool operator != (const Vectormath::Vector2 &u, const Vectormath::Vector2 &v) { return !(u == v); }
inline bool operator == (const Vectormath::Point2 &u, const Vectormath::Point2 &v) { return (u.getX() == v.getX() && u.getY() == v.getY()); }
inline bool operator != (const Vectormath::Point2 &u, const Vectormath::Point2 &v) { return !(u == v); }

inline bool operator == (const Vectormath::Vector3 &u, const Vectormath::Vector3 &v)
{
...
inline bool operator == (const Vectormath::Point3 &u, const Vectormath::Point3 &v)
{
#if VECTORMATH_MODE_SCALAR
	return !operator==(u, v);
#else
...
inline bool operator == (const Vectormath::Vector4 &u, const Vectormath::Vector4 &v)
{
#if VECTORMATH_MODE_SCALAR
	return
		u.getX() == v.getX() &&
...
inline bool operator == (const Vectormath::Quat &u, const Vectormath::Quat &v)
{
#if VECTORMATH_MODE_SCALAR
	return
		u.getX() == v.getX() &&
...
inline bool operator == (const Vectormath::Vector4d &u, const Vectormath::Vector4d &v)
{
	return
		u.getX() == v.getX() &&
		u.getY() == v.getY() &&
...
inline bool operator == (const Vectormath::Vector3d &u, const Vectormath::Vector3d &v)
{
	return
		u.getX() == v.getX() &&
		u.getY() == v.getY() &&
...
inline bool operator == (const Vectormath::IVector4 &u, const Vectormath::IVector4 &v)
{
#if VECTORMATH_MODE_SCALAR
	return
		u.getX() == v.getX() &&
...
#else
	//Compare the X, Y, Z, and W values
	return _mm_movemask_epi8(_mm_cmpeq_epi32(u.get128(),v.get128())) == 0xffff;
#endif
}
inline bool operator == (const Vectormath::UVector4 &u, const Vectormath::UVector4 &v)
{
#if VECTORMATH_MODE_SCALAR
	return
		u.getX() == v.getX() &&
...
inline bool operator == (const Vectormath::IVector3 &u, const Vectormath::IVector3 &v)
{
#if VECTORMATH_MODE_SCALAR
	return
		u.getX() == v.getX() &&
		...
		u.getZ() == v.getZ();
#else
	//Compare the X, Y, Z values, but mask away the result of W
	return (_mm_movemask_epi8(_mm_cmpeq_epi32(u.get128(),v.get128())) & 0xfff0) == 0xfff0;
#endif
}

inline bool operator == (const Vectormath::UVector3 &u, const Vectormath::UVector3 &v)
{
#if VECTORMATH_MODE_SCALAR
	return
		u.getX() == v.getX() &&
...
inline bool operator == (const Vectormath::Matrix4d &u, const Vectormath::Matrix4d &v)
{
	return
		u.getCol0() == v.getCol0() &&
		u.getCol1() == v.getCol1() &&
...
inline bool operator == (const Vectormath::Matrix4 &u, const Vectormath::Matrix4 &v)
{
	return
		u.getCol0() == v.getCol0() &&
		u.getCol1() == v.getCol1() &&
...
inline bool operator == (const Vectormath::Transform3 &u, const Vectormath::Transform3 &v)
{
	return
		u.getCol0() == v.getCol0() &&
		u.getCol1() == v.getCol1() &&
...
inline bool operator == (const Vectormath::Matrix3 &u, const Vectormath::Matrix3 &v)
{
	return
		u.getCol0() == v.getCol0() &&
		u.getCol1() == v.getCol1() &&
...
inline bool operator == (const Vectormath::Matrix3d &u, const Vectormath::Matrix3d &v)
{
	return
		u.getCol0() == v.getCol0() &&
		u.getCol1() == v.getCol1() &&
...
inline bool operator == (const Vectormath::AffineTransform &u, const Vectormath::AffineTransform &v)
{
	return
		u.rotation == v.rotation &&
		u.translation == v.translation &&
		u.scale == v.scale;
}
#if VECTORMATH_MODE_SCE
}}}}
...
inline int32_t getRandomInt()
{
	static std::mt19937 rng(TF_INITIAL_SEED);
	static std::uniform_int_distribution<uint32_t> uint_dist10(0, TF_RAND_MAX);
...
inline int randomInt(int mn, int mx) {
	//NAU_ASSERT(((long long)mx - (long long)mn) <= TF_RAND_MAX);
...
struct float3
{
    ...
    inline explicit float3(Vector3 xyz)							 : x(xyz.getX()), y(xyz.getY()), z(xyz.getZ()) {}
    inline explicit float3(Point3 xyz)							 : x(xyz.getX()), y(xyz.getY()), z(xyz.getZ()) {}
...
struct float4
{
	...
	inline explicit float4(Vector4 xyzw)							 : x(xyzw.getX()), y(xyzw.getY()), z(xyzw.getZ()), w(xyzw.getW()) {}
...
inline Point3 lerp(const Point3 &u, const Point3 &v, const float x) { return u + x * (v - u); }
...
inline void generateSpherePoints(std::vector<float3>& ppPoints, int numberOfDivisions, float radius = 1.0f)
{
	...
	// TODO: replace with best aligment
	ppPoints.resize(numberOfPoints * 2);
	float3* pPoints = (float3*)ppPoints.data(); //(float3*)tf_malloc(numberOfPoints * sizeof(float3) * 2);
...
inline void generateQuad(std::vector<float3>& ppPoints, float sideLength = 1.0f)
{
...
inline void generateCuboidPoints(std::vector<float3>& ppPoints, float width = 1.f, float height = 1.f, float depth = 1.f, Vector3 center = Vector3{ 0.f,0.f,0.f })
{
...
inline void generateBonePoints(std::vector<float3>& ppPoints, float widthRatio)
{
...
// Generates an array of vertices and normals for a bone of length = 1.f and width = widthRatio
inline void generateIndexedBonePoints(std::vector<IndexBonePoint>& ppPoints, float widthRatio, uint32_t boneCount, const int16_t* parentIndices)
{
	uint32_t numberOfPoints = 8 * 3 * boneCount;
	ppPoints.resize(numberOfPoints);
	IndexBonePoint* pPoints = ppPoints.data(); //(indexBonePoint*)tf_malloc(numberOfPoints * vertexSize);
	uint32_t vertexCounter = 0;
...
{	// Bounding box
	...
	inline void InitFromVertsSlow(const float3* verts, int num)
	{
		float3 f3minBounds = float3(FLT_MAX, FLT_MAX, FLT_MAX);
		float3 f3maxBounds = float3(-FLT_MAX, -FLT_MAX, -FLT_MAX);

		for (int i = 0; i < num; ++i)
		{
			const float3& vert = verts[i];

			f3minBounds = min(f3minBounds, vert);
			f3maxBounds = max(f3maxBounds, vert);
		}

		minBounds = Vector3(f3minBounds.x, f3minBounds.y, f3minBounds.z);
		maxBounds = Vector3(f3maxBounds.x, f3maxBounds.y, f3maxBounds.z);
	}
